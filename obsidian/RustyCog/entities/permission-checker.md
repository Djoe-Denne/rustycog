...
`rustycog::permission::PermissionChecker` is the single interface every service uses to ask "can this subject do this action on this resource?". It replaced the old per-service `PermissionEngine` + `PermissionsFetcher` pair.
