...
summary: >-
  Canonical map from RustyCog feature-gated modules to detailed references plus shared concept/entity/skill pages.
provenance:
  extracted: 0.9
  inferred: 0.06
  ambiguous: 0.04
...
This index is the canonical semantic map for RustyCog's unified crate layout.
...
- Root workspace members are now `rustycog` and `rustycog/rustycog-testing` for the framework layer.
- `rustycog/Cargo.toml` defines one package with feature flags (`core`, `command`, `config`, `db`, `events`, `outbox`, `http`, `permission`, `server`, `logger`, `test-utils`, `full`).
...
- `rustycog::outbox` (`outbox`) -> entities: [[entities/domain-event]], [[entities/event-publisher]], [[entities/db-connection-pool]] | concepts: [[concepts/event-driven-microservice-platform]], [[projects/aiforall/roadmap]]
