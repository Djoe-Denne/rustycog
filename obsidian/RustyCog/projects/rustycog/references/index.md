...
- The framework layer now has one package, `rustycog-framework`; integration fixtures are included behind its `testing` feature.
...
- `Cargo.toml` defines one package with feature flags (`core`, `command`, `config`, `db`, `events`, `outbox`, `http`, `permission`, `server`, `logger`, `testing`, `test-utils`, `full`).
...
- `rustycog::testing` maps to `rustycog-testing/src` and is packaged with `rustycog-framework`; `test-utils` is a compatibility alias for `testing`.
