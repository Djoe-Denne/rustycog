...
summary: rustycog::core defines the shared ServiceError and DomainError contracts that other RustyCog modules and services build on.
provenance:
  extracted: 0.88
  inferred: 0.05
  ambiguous: 0.07
...
- `ServiceError::category()`, `is_retryable()`, and `http_status_code()` centralize behavior that other modules reuse for retries and HTTP responses.
