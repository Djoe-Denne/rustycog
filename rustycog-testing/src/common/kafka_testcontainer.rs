...
impl TestKafka {
    ...
    /// # Errors
    ///
    /// Returns an error if the Kafka test container cannot be started or Kafka
    /// is not ready.
    ...
    /// # Errors
    ///
    /// Returns an error if waiting for the message fails.
    ...
    /// # Errors
    ///
    /// Returns an error if the test consumer cannot be created or polling fails.
    ...
    /// # Errors
    ///
    /// Returns an error if the test consumer cannot be created, polling fails,
    /// or the expected number of messages is not received within `max_wait_secs`.
...
#[allow(clippy::significant_drop_tightening)] // Mutex held for the whole init to avoid a second Kafka start.
...
fn register_kafka_cleanup_handler() {
...
impl TestKafkaFixture {
    ...
    /// # Errors
    ///
    /// Returns an error if the shared Kafka test instance cannot be created.
    pub async fn new() -> Result<Self, Box<dyn std::error::Error>> {
...
impl TestKafkaConsumer {
    ...
    /// # Errors
    ///
    /// Returns an error if the Kafka consumer cannot be created or cannot subscribe
    /// to the topic.
    ...
    /// # Errors
    ///
    /// Returns an error if consuming or decoding messages from the topic fails.
    pub async fn get_all_messages(
        &self,
        max_wait_secs: u64,
    ) -> Result<Vec<String>, Box<dyn std::error::Error>> {
    ...
    /// # Errors
    ///
    /// Returns an error if polling fails or the expected number of messages is
    /// not received within `max_wait_secs`.
    pub async fn wait_for_messages(
        &self,
        expected_count: usize,
        max_wait_secs: u64,
